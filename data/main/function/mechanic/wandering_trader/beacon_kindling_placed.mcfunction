execute at @n[type=item_frame,tag=beacon_kindling] run setblock ~ ~ ~ minecraft:campfire[signal_fire=true]
execute at @n[type=item_frame,tag=beacon_kindling] run particle minecraft:flame ~ ~.7 ~ .1 .1 .1 0.07 30
execute at @n[type=item_frame,tag=beacon_kindling] run playsound minecraft:item.firecharge.use block @a ~ ~ ~ 1
execute at @n[type=item_frame,tag=beacon_kindling] run playsound minecraft:entity.wither.spawn block @a ~ ~ ~ 0.5
